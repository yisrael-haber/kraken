    ...
    return `
        <form id="adopted-script-form" class="identity-setting wa-stack wa-gap-xs">
            ...
            <div class="form-actions wa-cluster wa-gap-xs">
                <wa-button variant="brand" appearance="filled" size="xs" type="submit" ${state.savingAdoptedScript ? 'loading' : ''} ${busy ? 'disabled' : ''}>Save</wa-button>
            </div>
    ...
    return `
        <div class="identity-capture wa-stack wa-gap-2xs wa-align-items-end">
            <div class="identity-capture__actions wa-cluster wa-gap-xs">
                ...
                ` : `
                    <wa-button variant="brand" appearance="filled" size="xs" type="button" data-start-adopted-recording ${state.startingAdoptedRecording ? 'loading' : ''} ${busy ? 'disabled' : ''}>Start capture</wa-button>
                `}
            </div>
        ...
        <div class="operation-panel">
            <form id="adopted-ip-dns-form" class="operation-form wa-grid wa-gap-s">
                ...
                <div class="form-actions wa-cluster wa-gap-xs wa-align-self-end">
                    <wa-button variant="brand" appearance="filled" size="xs" type="submit" ${busy ? 'loading disabled' : ''}>Resolve</wa-button>
                </div>
            </form>
        </div>
    `;
...
function renderDNSResultPanel(state) {
    ...
    return `
        <section class="operation-result dns-result wa-stack wa-gap-s">
            <header class="dns-result__header wa-split wa-gap-s">
                <h2 class="wa-heading-s">Result</h2>
        ...
        {label: 'Query', value: `${result.name || ''} ${result.type || ''}`.trim(), code: true},
        {label: 'Transport', value: String(result.transport || '').toUpperCase()},
    ...
    return `
        <div class="operation-panel">
            <form id="adopted-ip-ping-form" class="operation-form wa-grid wa-gap-s">
                ...
                <div class="form-actions wa-cluster wa-gap-xs wa-align-self-end">
                    <wa-button variant="brand" appearance="filled" size="xs" type="submit" ${busy ? 'loading disabled' : ''}>Ping</wa-button>
                </div>
            </form>
        </div>
    `;
...
function renderPingResultPanel(state) {
    ...
    `);
    return `
        <section class="operation-result ping-result wa-stack wa-gap-s">
...
function renderServiceForm(definition, state) {
    ...
    return `
        <form id="adopted-service-form" class="service-start-form wa-stack wa-gap-s">
            ...
            <div class="form-actions wa-cluster wa-gap-xs">
                <wa-button variant="brand" appearance="filled" size="xs" type="submit" ${starting ? 'loading' : ''} ${busy ? 'disabled' : ''}>Start</wa-button>
            </div>
        </form>
    `;
}
...
function renderInfoTab({details, item, state}) {
    ...
    return `
        <div class="content-column identity-workspace wa-stack wa-gap-m">
            <div class="identity-overview wa-split wa-gap-m">
                <div class="identity-overview__details">
                    ${renderInlineMeta(identityDetails)}
                </div>
                ${renderInfoCaptureControl(current, state)}
            </div>
            <section class="identity-settings-section wa-stack wa-gap-s" aria-labelledby="identity-settings-heading">
                <h2 id="identity-settings-heading" class="wa-heading-m">Settings</h2>
                <div class="identity-settings wa-grid wa-gap-s">
                    ...
                    <form id="adopted-mtu-form" class="identity-setting wa-stack wa-gap-xs">
                        ...
                        <div class="form-actions wa-cluster wa-gap-xs">
                            <wa-button variant="brand" appearance="filled" size="xs" type="submit" ${state.updatingAdoptedMTU ? 'loading' : ''} ${busy ? 'disabled' : ''}>Save</wa-button>
                        </div>
                    </form>
                </div>
            </section>
...
function renderServicesTab(details, state) {
    ...
    return `
        <div class="content-column services-workspace wa-stack wa-gap-m">
            ...
            <section class="services-live wa-stack wa-gap-xs" aria-labelledby="live-services-heading">
                <div class="section-heading wa-split wa-gap-xs">
                    <h2 id="live-services-heading" class="wa-heading-s">Live services</h2>
                    <div class="services-live__status wa-cluster wa-gap-xs">
                        ${state.adoptedServiceNotice ? `<span class="service-action-status wa-caption-xs" role="status" aria-live="polite">${escapeHTML(state.adoptedServiceNotice)}</span>` : ''}
                        ${liveServiceCount ? pill(`${liveServiceCount} running`, 'success') : ''}
                    </div>
                </div>
                ${liveServiceCount
        ? renderLiveServicesTable(details, state)
        : '<p class="wa-caption-s">No services running.</p>'}
            </section>
            <section class="service-launcher wa-stack wa-gap-xs" aria-labelledby="service-launcher-heading">
                <h2 id="service-launcher-heading" class="wa-heading-s">Start a service</h2>
...
export function renderAdoptedIPAddressView({details, item, state}) {
    if (!item) {
        return `
            <section class="wa-stack wa-gap-m">
                ${renderModuleTopbar('', {backToIdentities: true})}
                ${renderStateLayout('No adopted IP selected', 'Return to Identities and open an active identity.')}
            </section>
        `;
    }
    ...
    return `
        <section class="wa-stack wa-gap-m">
            ${renderModuleTopbar(item.label || item.ip || 'Adopted identity', {backToIdentities: true})}
...
export function renderOperationsModule({state}) {
    ...
    return `
        <section class="wa-stack wa-gap-m">
            ${renderModuleTopbar('Operations')}
            <div class="single-panel-layout single-panel-layout--wide">
